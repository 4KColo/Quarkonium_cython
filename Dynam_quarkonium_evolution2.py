	...
	def run(self, dt = 0.04):
		...
		if self.recombine == True:
			...
			for i in range(len_Q):					# loop over Q
				...
				rate_reco1P_ineq = []
				rate_reco1P_ineg = []
				for j in range(len_Qbar):		# loop over Qbar
					# positions in lab frame
					xQ = self.Qlist['3-position'][i]
					xQbar = self.Qbarlist['3-position'][j]
					x_rel = xQ - xQbar
					x_CM = 0.5*( xQ + xQbar )					
					...
					if  T_Vxyz[0] >= Tc and T_Vxyz[0] <= T_1S and j not in delete_Qbar:
						r_rel = np.sqrt(np.sum(x_rel**2))
						pQ_lab = self.Qlist['4-momentum'][i]
						pQbar_lab = self.Qbarlist['4-momentum'][j]
