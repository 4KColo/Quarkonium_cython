...
// Matrix element of |<1S|r|Psi(p)>|^2, (Coulomb interaction)
double Matrix1S(double p){
    double eta = eta_scale/(p+small_number);
    double x = a_B * p;
...
// Matrix element of |<2S|r|Psi(p)>|^2, (Coulomb interaction)
...
//------------------------------------------- Quarkonium 1P dissociation ----------------------------------
// Matrix element of |<1P|r|Psi(p)>|^2, (Coulomb interaction)
double Matrix1P(double p){
    double eta = eta_scale/(p+small_number);
    double x = 2.0 * a_B * p;
    double Numerator_1 = Matrix1P_prefactor * eta * std::exp(4.0*eta*std::atan(x)-TwoPi*eta);
    double Factor1 = eta*(eta*eta-2.)*pow(x,3) + 3.*(2.*eta*eta-1.)*x*x + 9.*eta*x + 3.;
    double Factor2 = 3.+eta*x;
    double Numerator_2 = pow(Factor1,2) + (pow(eta,4)+5.*pow(eta,2)+4.)*pow(x,4)*pow(Factor2,2);
    double Denominator = pow(1.0+x*x, 8) * ( 1.0 - std::exp(-TwoPi*eta) );
    return Numerator_1*Numerator_2/Denominator;
}
...
double p2Matrix1P(double p){
    return p*p*Matrix1P(p);
}

const double max_p2Matrix1P = find_max_noparams(&p2Matrix1P, small_number, p_1Psam);

//---------------------- gluo-dissociation -----------------------
// Cross-section of 1P -> Psi(p)
double Xsec1P(double q){
    if (q <= E1P) return 0.;
    double p = std::sqrt(M*(q-E1P));
    return Xsec1S_v2_prefactor*q*p*Matrix1P(p);
}
...
double dRdq_1P_gluon(double q, void * params_){
    ...
    double k1 = params[0]; //k1 is gamma*(1.+v)/T;
    double k2 = params[1]; //k2 is gamma*(1.-v)/T;
    return q*Xsec1P(q)*(fac1(q*k1) - fac1(q*k2));
}

double dRdq_1P_gluon_small_v(double q, void * params_){
    double * params = static_cast<double *>(params_);
    double T = params[0];
    return q*q*Xsec1P(q)*nB(q/T);
}
...
double qdRdq_1P_gluon_u(double u, void * params_){
    ...
    new_params[0] = gamma*(1.+v)/T;
    new_params[1] = gamma*(1.-v)/T;
    double q = E1P*std::exp(u);
    return q*dRdq_1P_gluon(q, new_params);
}
...
double R1P_decay_gluon(double vabs, double T){
    if (vabs < small_number){
        double result, error, qmin=E1P, qmax=E1P*100.;
        gsl_integration_workspace *w = gsl_integration_workspace_alloc(1);
        double * params = new double[1];
        params[0] = T;
        gsl_function F;
        F.function = dRdq_1P_gluon_small_v;
        F.params = params;
        gsl_integration_qag(&F, qmin, qmax, 0, 1e-6, 1000, 6, w, &result, &error);
        delete [] params;
        gsl_integration_workspace_free(w);
...
double S1P_decay_gluon_q(double v, double T, double maximum){
    ...
    do{
        u = dist_u(gen);
        result = qdRdq_1P_gluon_u(u, params)/maximum;
    } while( rejection(gen) > result );
    return E1P*std::exp(u);
}
...
double S1P_decay_gluon_final_p(double q){
    return std::sqrt((q-E1P)*M);
}

std::vector<double> S1P_decay_gluon(double v, double T, double maximum){
    double q = S1P_decay_gluon_q(v, T, maximum);
    double cos = S1P_decay_gluon_costheta(q, v, T);
    double phi = sample_inel(gen)*TwoPi;
    double p_rel = S1P_decay_gluon_final_p(q);
    double cos_rel = sample_cos(gen);
    double phi_rel = sample_inel(gen)*TwoPi;
    std::vector<double> momentum_gluon(3);
    std::vector<double> momentum_rel(3);
...
double dRdp1dp2_1P_decay_ineq(double x[5], size_t dim, void * params_){
    ...
    if (p2 <= 0.0){
        ...
        double part_angle = s1*s2*std::cos(phi)+c1*c2;
        double prop = p1*p2*(1.+part_angle)/( p1*p1+p2*p2-2.*p1*p2*part_angle );
        return phase1 * phase2 * Prel*Prel * Matrix1P(Prel) * prop;
        // omit a prefactor and a 1/gamma, add them in the rate calculation: R1P_decay_ineq
    }
}
double R1P_decay_ineq(double v, double T){
    ...
    double result, error;
    double p1up = 15.*T/std::sqrt(1.-v);
    double xl[5] = { E1P, -1., 0., -1., 0. };
    double xu[5] = { p1up, 1., p_1Psam, 1., TwoPi };
    gsl_monte_function F;
    F.f = &dRdp1dp2_1P_decay_ineq;
    F.dim = 5;
    F.params = params;
    gsl_rng * r = gsl_rng_alloc (gsl_rng_default);
    gsl_monte_vegas_state *w = gsl_monte_vegas_alloc (5); // create VEGAS workstation
...
double S1P_decay_ineq_p1_important(double p1low, double p1up, double result_max, void * params_){
    double * params = static_cast<double *>(params_);
    //double result_max = find_max(&f_p1_decay1P_important, params, p1low, p1up);
    double result_try, p1_try;
    do{
        p1_try = sample_inel(gen)*(p1up-p1low) + p1low;
        result_try = f_p1_decay1P_important(p1_try, params);
    } while(rejection(gen)*result_max > result_try);
    return p1_try;
}
...
std::vector<double> S1P_decay_ineq(double v, double T, double maximum){
    ...
    do{
        do{
            p_rel = sample_inel(gen)*p_1Psam;
        } while(rejection(gen)*max_p2Matrix1P > p2Matrix1P(p_rel));
        
        p1_try = S1P_decay_ineq_p1_important(p1low, p1up, maximum, params_p1);
        p2_try = p1_try - E1P - p_rel*p_rel/M;
        if (p2_try <= 0.0){
            result_try = 0.0;
        }
        else{
            c1_try = S1P_decay_ineq_cos1(p1_try, params_c1);
            c2_try = sample_cos(gen);
            s1_try = std::sqrt(1.-c1_try*c1_try);
            s2_try = std::sqrt(1.-c2_try*c2_try);
            phi_try = sample_inel(gen)*TwoPi;
            p1p2_try = p1_try/p2_try;   // p1_try/p2_try
            p1p2 = (p1_try-E1P)/p1_try;
            c_phi = std::cos(phi_try);
            s_phi = std::sin(phi_try);
            part_angle = s1_try*s2_try*c_phi + c1_try*c2_try;
            result_try = (1.+part_angle)/(p1p2_try + 1./p1p2_try - 2.*part_angle)/2.0*(p1p2 + 1./p1p2 - 2.0) * nFminus1(gamma*(1.+v*c2_try)*p2_try/T)/p1p2_try;
...
double R1P_decay_ineg(double v, double T){
    ...
    double q1up = 15.*T/std::sqrt(1.-v);
    double xl[5] = { E1P, -1., 0., -1., 0. };
    double xu[5] = { q1up, 1., p_1Psam, 1., TwoPi };
    gsl_monte_function F;
    F.f = &dRdq1dq2_1P_decay_ineg;
    F.dim = 5;
    F.params = params;
    gsl_rng * r = gsl_rng_alloc (gsl_rng_default);
    gsl_monte_vegas_state *w = gsl_monte_vegas_alloc (5); // create VEGAS workstation
...
double S1P_decay_ineg_q1_important(double q1low, double q1up, double result_max, void * params_){
    double * params = static_cast<double *>(params_);
    //double result_max = find_max(&f_q1_decay1P_important, params, q1low, q1up);
    double result_try, q1_try;
    do{
        q1_try = sample_inel(gen)*(q1up-q1low) + q1low;
        result_try = f_q1_decay1P_important(q1_try, params);
    } while(rejection(gen)*result_max > result_try);
    return q1_try;
}
...
std::vector<double> S1P_decay_ineg(double v, double T, double maximum){
    ...
    double q1low = E1P;
    double q1up = 15.*T/std::sqrt(1.-v);
    double result_max = T/E1P/gamma/(1.-v);
    ...
    do{
        ...
        q1_try = S1P_decay_ineg_q1_important(q1low, q1up, maximum, params_q1);
        q2_try = q1_try - E1P - p_rel*p_rel/M;
        if (q2_try <= 0.0){
            result_try = 0.0;
        }
        else{
            c1_try = S1P_decay_ineg_cos1(q1_try, params_c1);
            c2_try = sample_cos(gen);
            s1_try = std::sqrt(1.-c1_try*c1_try);
            s2_try = std::sqrt(1.-c2_try*c2_try);
            phi_try = sample_inel(gen)*TwoPi;
            q1q2_try = q1_try/q2_try;   // q1_try/q2_try
            q1q2 = (q1_try-E1P)/q1_try;
            c_phi = std::cos(phi_try);
            s_phi = std::sin(phi_try);
            part_angle = s1_try*s2_try*c_phi + c1_try*c2_try;
            result_try = nBplus1(gamma*(1.+v*c2_try)*q2_try/T)/q1q2_try * (q1q2_try  + 1./q1q2_try + 2.0)/(q1q2_try + 1./q1q2_try - 2.0*part_angle) * (1.+part_angle)/2.0 / (1.0 + 4./(q1q2 + 1./q1q2 - 2.0) );
...
//-------------------------------------------- end of Quarkonium 1P dissociation --------------------------------------------



// ------------------------------------------- Quark and anti-quark 1P recombination ----------------------------------------
//---------------------- gluon induced recombination -----------------------
// we define rate * vol (RV) = v_rel * cross section in GeV*fm^3; p is the QQbar relative momentum
// rate * vol for a specific color, no summation over colors
double RV1P_reco_gluon(double v, double T, double p){
    double q = p*p/M + E1P;
    double reco = RV1S_prefactor * pow(q,3) * Matrix1P(p)* pow(InverseFermiToGeV,3);
    // convert GeV^-2 to GeV * fm^3
    if (v < small_number){
        double enhencement = nBplus1(q/T);
        return 2.* reco * enhencement;
...
std::vector<double> S1P_reco_gluon(double v, double T, double p){
    double q = S1P_reco_gluon_q(p);
    double cos = S1P_reco_gluon_costheta(v, T, q);
    double phi = sample_inel(gen)*TwoPi;
    std::vector<double> p1P_final(3);
    p1P_final = polar_to_cartisian1(q, cos, phi);
    p1P_final = subtract_real_gluon( p1P_final );
    return p1P_final;
}
//------------------- end of gluon induced recombination ---------------------
...
double S1P_reco_ineq_p1_important(double p1low, double p1up, double result_max, void * params_){
    double * params = static_cast<double *>(params_);
    //double result_max = find_max(&f_p1_reco1P_important, params, p1low, p1up);
    double result_try, p1_try;
    do{
        p1_try = sample_inel(gen)*(p1up-p1low) + p1low;
        result_try = f_p1_reco1P_important(p1_try, params);
    } while(rejection(gen)*result_max > result_try);
    return p1_try;
}
...
std::vector<double> S1P_reco_ineq(double v, double T, double p, double maximum){
    ...
    do{
        p1_try = S1P_reco_ineq_p1_important(p1low, p1up, maximum, params_p1);
        c1_try = S1P_decay_ineq_cos1(p1_try, params_c1);
        p2_try = p1_try + E_rel + E1P;
        c2_try = sample_cos(gen);
        s1_try = std::sqrt(1.-c1_try*c1_try);
        s2_try = std::sqrt(1.-c2_try*c2_try);
        phi_try = sample_inel(gen)*TwoPi;
    ...
    p1_final = polar_to_cartisian2(p1_try, c1_try, s1_try, 1.0, 0.0);
    p2_final = polar_to_cartisian2(p2_try, c2_try, s2_try, c_phi, s_phi);
    p1P_final = subtract_virtual_gluon(p1_final, p2_final);
    return p1P_final;
}
...
double S1P_reco_ineg_q1_important(double q1low, double q1up, double result_max, void * params_){
    double * params = static_cast<double *>(params_);
    //double result_max = find_max(&f_q1_reco1P_important, params, q1low, q1up);
    double result_try, q1_try;
    do{
        q1_try = sample_inel(gen)*(q1up-q1low) + q1low;
        result_try = f_q1_reco1P_important(q1_try, params);
    } while(rejection(gen)*result_max > result_try);
    return q1_try;
}
...
std::vector<double> S1P_reco_ineg(double v, double T, double p, double maximum){
    ...
    double q1up = 15.*T/std::sqrt(1.-v);
    double E_rel = p*p/M;
    double result_max = nBplus1(gamma*(1.0-v)*(E1P+E_rel)/T);
    ...
    do{
        q1_try = S1P_reco_ineg_q1_important(q1low, q1up, maximum, params_q1);
        c1_try = S1P_decay_ineg_cos1(q1_try, params_c1);
        q2_try = q1_try + E_rel + E1P;
        c2_try = sample_cos(gen);
        s1_try = std::sqrt(1.-c1_try*c1_try);
        s2_try = std::sqrt(1.-c2_try*c2_try);
        phi_try = sample_inel(gen)*TwoPi;
    ...
    q1_final = polar_to_cartisian2(q1_try, c1_try, s1_try, 1.0, 0.0);
    q2_final = polar_to_cartisian2(q2_try, c2_try, s2_try, c_phi, s_phi);
    p1P_final = subtract_virtual_gluon(q1_final, q2_final);
    return p1P_final;
}
...
std::vector<double> p3top4_quarkonia_1P(std::vector<double> p3){
    std::vector<double> p4(4);
    p4[0] = std::sqrt(M1P*M1P + p3[0]*p3[0] + p3[1]*p3[1] + p3[2]*p3[2]);
    p4[1] = p3[0];
    p4[2] = p3[1];
    p4[3] = p3[2];
    return p4;
