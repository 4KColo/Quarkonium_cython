...
cdef extern from "../src/DisRec.h":
	...
	cdef double R1P_decay_gluon(double vabs, double T)
	...
	cdef double dRdq_1P_gluon(double q, void * params_)
	cdef double dRdq_1P_gluon_small_v(double q, void * params_)
	cdef double qdRdq_1P_gluon_u(double u, void * params_)
	cdef vector[double] S1P_decay_gluon(double v, double T, double maximum)
	...
	cdef double R1P_decay_ineq(double v, double T)
	...
	cdef double f_p1_decay1P_important(double p1, void * params_)
	...
	cdef vector[double] S1P_decay_ineq(double v, double T, double maximum)
	cdef vector[double] S1P_decay_ineq_test(double v, double T, double maximum)
	...
	cdef double R1P_decay_ineg(double v, double T)
	...
	cdef double f_q1_decay1P_important(double q1, void * params_)
	...
	cdef double RV1P_reco_gluon(double v, double T, double p)
	cdef double dist_position_1S(double r)
	cdef double dist_position_2S(double r)
	cdef double dist_position_1P(double r)
	...
	cdef vector[double] S1P_reco_gluon(double v, double T, double p)
	...
	cdef double RV1P_reco_ineq(double v, double T, double p)
	...
	cdef double f_p1_reco1P_important(double p1, void * params_)
	...
	cdef vector[double] S1P_reco_ineq(double v, double T, double p, double maximum)
	cdef vector[double] S1P_reco_ineq_test(double v, double T, double p, double maximum)
	...
	cdef double RV1P_reco_ineg(double v, double T, double p)
	...
	cdef double f_q1_reco1P_important(double q1, void * params_)
	...
	cdef vector[double] S1P_reco_ineg(double v, double T, double p, double maximum)
	cdef vector[double] S1P_reco_ineg_test(double v, double T, double p, double maximum)
...
def pyR1P_decay_gluon(double vabs, double T):
	return R1P_decay_gluon(vabs, T)

def pyR1P_decay_ineq(double vabs, double T):
	return R1P_decay_ineq(vabs, T)

def pyR1P_decay_ineg(double vabs, double T):
	return R1P_decay_ineg(vabs, T)
		
def pyRV1P_reco_gluon(double vabs, double T, double p_rel):
	return RV1P_reco_gluon(vabs, T, p_rel)

def pyRV1P_reco_ineq(double vabs, double T, double p_rel):
	return RV1P_reco_ineq(vabs, T, p_rel)
	
def pyRV1P_reco_ineg(double vabs, double T, double p_rel):
	return RV1P_reco_ineg(vabs, T, p_rel)
...
pyR1P_decay_gluon = np.vectorize(pyR1P_decay_gluon)
pyR1P_decay_ineq = np.vectorize(pyR1P_decay_ineq)
pyR1P_decay_ineg = np.vectorize(pyR1P_decay_ineg)
pyRV1P_reco_gluon = np.vectorize(pyRV1P_reco_gluon)
pyRV1P_reco_ineq = np.vectorize(pyRV1P_reco_ineq)
pyRV1P_reco_ineg = np.vectorize(pyRV1P_reco_ineg)
...
cdef class DisRec(object):
	...
	cdef np.ndarray T_R1P_decay_gluon
	cdef np.ndarray T_R1P_decay_ineq
	cdef np.ndarray T_R1P_decay_ineg
	cdef np.ndarray T_qdRdq_1P_gluon_max
	cdef np.ndarray T_RV1P_reco_gluon
	cdef np.ndarray T_RV1P_reco_ineq
	cdef np.ndarray T_RV1P_reco_ineg
	cdef np.ndarray T_S1P_decay_ineq_max
	cdef np.ndarray T_S1P_decay_ineg_max
	cdef np.ndarray T_S1P_reco_ineq_max
	cdef np.ndarray T_S1P_reco_ineg_max
	...
	def __cinit__(self, table_folder='tables', overwrite=False):
		...
		if (gpname in f) and (not overwrite):
			...
			print ("loading Upsilon(1P)+g <-> b+bbar rate table")
			self.T_R1P_decay_gluon = gp['R1P_decay_gluon'].value
			self.T_qdRdq_1P_gluon_max = gp['qdRdq_1P_gluon_max'].value
			self.T_RV1P_reco_gluon = gp['RV1P_reco_gluon'].value
			...
			print ("loading Upsilon(1P)+q <-> b+bbar+q rate table")
			self.T_R1P_decay_ineq = gp['R1P_decay_ineq'].value
			self.T_RV1P_reco_ineq = gp['RV1P_reco_ineq'].value
			self.T_S1P_decay_ineq_max = gp['S1P_decay_ineq_max'].value
			self.T_S1P_reco_ineq_max = gp['S1P_reco_ineq_max'].value
			...
			print ("loading Upsilon(1P)+g <-> b+bbar+g rate table")
			self.T_R1P_decay_ineg = gp['R1P_decay_ineg'].value
			...
			self.vmin = 0.001; self.vmax = 0.999; self.N_v = 50
			self.Tmin = 0.15; self.Tmax = 0.55; self.N_T = 30
			# here we use p_rel in GeV, we need p_rel lower bound small enough, otherwise 2S and 1P recombine would be problematic
			self.p_rel_log_min = -4.0; self.p_rel_log_max = 2.5; self.N_p_rel = 80
			...
			print ("generating Upsilon(1P)+g -> b+bbar rate table")
			self.T_R1P_decay_gluon = np.transpose(pyR1P_decay_gluon(grid_v, grid_T))
			self.T_qdRdq_1P_gluon_max = np.zeros_like(self.T_R1P_decay_gluon)
			...
			print ("generating Upsilon(1P)+q -> b+bbar+q rate table")
			self.T_R1P_decay_ineq = np.transpose(pyR1P_decay_ineq(grid_v, grid_T))
			self.T_S1P_decay_ineq_max = np.zeros_like(self.T_R1P_decay_ineq)
			...
			print ("generating Upsilon(1P)+g -> b+bbar+g rate table")
			self.T_R1P_decay_ineg = np.transpose(pyR1P_decay_ineg(grid_v, grid_T))
			self.T_S1P_decay_ineg_max = np.zeros_like(self.T_R1P_decay_ineg)
			...
			for iv, v in enumerate(varray):
				...
				for iT, T in enumerate(Tarray):
					...
					self.T_qdRdq_1S_gluon_max[iv, iT] = find_max(&qdRdq_1S_gluon_u, params, small_number, 3.)
					self.T_qdRdq_2S_gluon_max[iv, iT] = find_max(&qdRdq_2S_gluon_u, params, small_number, 1.2354)
					self.T_qdRdq_1P_gluon_max[iv, iT] = find_max(&qdRdq_1P_gluon_u, params, small_number, 3.)
					...
					self.T_S1P_decay_ineq_max[iv, iT] = find_max(&f_p1_decay1P_important, params_decay, E1P, uplim)
					...
					self.T_S1P_decay_ineg_max[iv, iT] = find_max(&f_q1_decay1P_important, params_decay, E1P, uplim)
			...
			print ("generating b+bbar -> Upsilon(1P)+g rate*vol table, vol in fm^3")
			self.T_RV1P_reco_gluon = pyRV1P_reco_gluon(grd_v, grd_T, grd_p_rel).transpose(1,0,2)
			...
			print ("generating b+bbar+q -> Upsilon(1P)+q rate*vol table, vol in fm^3")
			self.T_RV1P_reco_ineq = pyRV1P_reco_ineq(grd_v, grd_T, grd_p_rel).transpose(1,0,2)
			self.T_S1P_reco_ineq_max = np.zeros_like(self.T_RV1P_reco_ineq)
			...
			print ("generating b+bbar+g -> Upsilon(1P)+g rate*vol table, vol in fm^3")
			self.T_RV1P_reco_ineg = pyRV1P_reco_ineg(grd_v, grd_T, grd_p_rel).transpose(1,0,2)
			self.T_S1P_reco_ineg_max = np.zeros_like(self.T_RV1P_reco_ineg)
			...
			for iv, v in enumerate(varray):
				...
				for iT, T in enumerate(Tarray):
					...
					for iPrel, Prel in enumerate(p_relarray):
						...
						self.T_S1P_reco_ineq_max[iv, iT, iPrel] = find_max(&f_p1_reco1P_important, params_reco, small_number, uplim)
						...
						self.T_S1P_reco_ineg_max[iv, iT, iPrel] = find_max(&f_q1_reco1P_important, params_reco, small_number, uplim)
			...
			gp.create_dataset('R1P_decay_gluon', data=self.T_R1P_decay_gluon)
			gp.create_dataset('R1P_decay_ineq', data=self.T_R1P_decay_ineq)
			gp.create_dataset('R1P_decay_ineg', data=self.T_R1P_decay_ineg)
			gp.create_dataset('qdRdq_1P_gluon_max', data=self.T_qdRdq_1P_gluon_max)
			gp.create_dataset('RV1P_reco_gluon', data = self.T_RV1P_reco_gluon)
			gp.create_dataset('RV1P_reco_ineq', data = self.T_RV1P_reco_ineq)
			gp.create_dataset('RV1P_reco_ineg', data = self.T_RV1P_reco_ineg)
			...
			gp.create_dataset('S1P_decay_ineq_max', data = self.T_S1P_decay_ineq_max)
			...
			gp.create_dataset('S1P_decay_ineg_max', data = self.T_S1P_decay_ineg_max)
			...
			gp.create_dataset('S1P_reco_ineq_max', data = self.T_S1P_reco_ineq_max)
			...
			gp.create_dataset('S1P_reco_ineg_max', data = self.T_S1P_reco_ineg_max)
	...
	# 1P real gluon dissociation
	cpdef get_R1P_decay_gluon(self, double v, double T):
		return interp2d(self.T_R1P_decay_gluon, v, T, 
						self.vmin, self.vmax, self.dv, self.N_v,
						self.Tmin, self.Tmax, self.dT, self.N_T)
						
	# 1P real gluon dissociation max integrand
	cpdef get_qdRdq_1P_gluon_max(self, double v, double T):
		return interp2d(self.T_qdRdq_1P_gluon_max, v, T, 
						self.vmin, self.vmax, self.dv, self.N_v,
						self.Tmin, self.Tmax, self.dT, self.N_T)
						
	# 1P inelastic quark dissociation
	cpdef get_R1P_decay_ineq(self, double v, double T):
		return interp2d(self.T_R1P_decay_ineq, v, T, 
						self.vmin, self.vmax, self.dv, self.N_v,
						self.Tmin, self.Tmax, self.dT, self.N_T)

	# 1P inelastic gluon dissociation
	cpdef get_R1P_decay_ineg(self, double v, double T):
		return interp2d(self.T_R1P_decay_ineg, v, T, 
						self.vmin, self.vmax, self.dv, self.N_v,
						self.Tmin, self.Tmax, self.dT, self.N_T)
							
	# 1P gluon recombination
	cpdef get_R1P_reco_gluon(self, double v, double T, double p_rel, double r):
		small_number = 0.000001		# add this small number to p_rel in case p_rel = 0
		cdef double Rvol = interp3d(self.T_RV1P_reco_gluon, v, T, np.log(p_rel+small_number),
									self.vmin, self.vmax, self.dv, self.N_v,
									self.Tmin, self.Tmax, self.dT, self.N_T,
									self.p_rel_log_min, self.p_rel_log_max, self.dp_rel, self.N_p_rel)
	...
	# 1P inelastic quark recombination
	cpdef get_R1P_reco_ineq(self, double v, double T, double p_rel, double r):
		small_number = 0.000001		# add this small number to p_rel in case p_rel = 0
		cdef double Rvol = interp3d(self.T_RV1P_reco_ineq, v, T, np.log(p_rel+small_number),
									self.vmin, self.vmax, self.dv, self.N_v,
									self.Tmin, self.Tmax, self.dT, self.N_T,
									self.p_rel_log_min, self.p_rel_log_max, self.dp_rel, self.N_p_rel)
		return Rvol * dist_position_1P(r)	

	# 1P inelastic gluon recombination
	cpdef get_R1P_reco_ineg(self, double v, double T, double p_rel, double r):
		small_number = 0.000001		# add this small number to p_rel in case p_rel = 0
		cdef double Rvol = interp3d(self.T_RV1P_reco_ineg, v, T, np.log(p_rel+small_number),
									self.vmin, self.vmax, self.dv, self.N_v,
									self.Tmin, self.Tmax, self.dT, self.N_T,
									self.p_rel_log_min, self.p_rel_log_max, self.dp_rel, self.N_p_rel)
		return Rvol * dist_position_1P(r)	
	...
	# 1P gluon dissociation	
	cpdef vector[double] sample_S1P_decay_gluon(self, double v, double T):
		cdef vector[double] pQpQbar, pQ, pQbar
		pQpQbar = S1P_decay_gluon(v, T, self.get_qdRdq_1P_gluon_max(v, T))
		pQ = np.array(p3top4_Q(pQpQbar[0:3]))
		pQbar = np.array(p3top4_Q(pQpQbar[3:6]))
		return np.concatenate((pQ, pQbar), axis=0)
	...
	# 1P inelastic quark dissociation
	# max
	cpdef get_S1P_decay_ineq_max(self, double v, double T):
		return interp2d(self.T_S1P_decay_ineq_max, v, T, 
						self.vmin, self.vmax, self.dv, self.N_v,
						self.Tmin, self.Tmax, self.dT, self.N_T)
						
	cpdef vector[double] sample_S1P_decay_ineq(self, double v, double T):
		cdef vector[double] pQpQbar, pQ, pQbar
		if v < 0.999:
			pQpQbar = S1P_decay_ineq(v, T, self.get_S1P_decay_ineq_max(v, T))
		else:
			pQpQbar = S1P_decay_gluon(v, T, self.get_qdRdq_1P_gluon_max(v, T))
		pQ = np.array(p3top4_Q(pQpQbar[0:3]))
		pQbar = np.array(p3top4_Q(pQpQbar[3:6]))
	...
	# 1P inelastic gluon dissociation
	# max
	cpdef get_S1P_decay_ineg_max(self, double v, double T):
		return interp2d(self.T_S1P_decay_ineg_max, v, T, 
						self.vmin, self.vmax, self.dv, self.N_v,
						self.Tmin, self.Tmax, self.dT, self.N_T)
						
	cpdef vector[double] sample_S1P_decay_ineg(self, double v, double T):
		cdef vector[double] pQpQbar, pQ, pQbar
		if v < 0.999:
			pQpQbar = S1P_decay_ineg(v, T, self.get_S1P_decay_ineg_max(v, T))
		else:
			pQpQbar = S1P_decay_gluon(v, T, self.get_qdRdq_1P_gluon_max(v, T))
		pQ = np.array(p3top4_Q(pQpQbar[0:3]))
		pQbar = np.array(p3top4_Q(pQpQbar[3:6]))
	...
	# 1P gluon recombination
	cpdef vector[double] sample_S1P_reco_gluon(self, double v, double T, double p_rel):
		cdef vector[double] p1P
		p1P = S1P_reco_gluon(v, T, p_rel)
		return np.array(p3top4_quarkonia_1P(p1P))	# convert p3 to p4
		
	# 1P inelastic quark recombination
	# max
	cpdef get_S1P_reco_ineq_max(self, double v, double T, double p_rel):
		return interp3d(self.T_S1P_reco_ineq_max, v, T, np.log(p_rel+small_number),
									self.vmin, self.vmax, self.dv, self.N_v,
									self.Tmin, self.Tmax, self.dT, self.N_T,
									self.p_rel_log_min, self.p_rel_log_max, self.dp_rel, self.N_p_rel)
									
	cpdef vector[double] sample_S1P_reco_ineq(self, double v, double T, double p_rel):
		cdef vector[double] p1P
		if v < 0.999:
			p1P = S1P_reco_ineq(v, T, p_rel, self.get_S1P_reco_ineq_max(v, T, p_rel))
		else:
			p1P = S1P_reco_gluon(v, T, p_rel)
		return np.array(p3top4_quarkonia_1P(p1P))	# convert p3 to p4
	...
	# 1P inelastic gluon recombination
	# max
	cpdef get_S1P_reco_ineg_max(self, double v, double T, double p_rel):
		return interp3d(self.T_S1P_reco_ineg_max, v, T, np.log(p_rel+small_number),
									self.vmin, self.vmax, self.dv, self.N_v,
									self.Tmin, self.Tmax, self.dT, self.N_T,
									self.p_rel_log_min, self.p_rel_log_max, self.dp_rel, self.N_p_rel)
									
	cpdef vector[double] sample_S1P_reco_ineg(self, double v, double T, double p_rel):
		cdef vector[double] p1P
		if v < 0.999:
			p1P = S1P_reco_ineg(v, T, p_rel, self.get_S1P_reco_ineg_max(v, T, p_rel))
		else:
			p1P = S1P_reco_gluon(v, T, p_rel)
		return np.array(p3top4_quarkonia_1P(p1P))	# convert p3 to p4
